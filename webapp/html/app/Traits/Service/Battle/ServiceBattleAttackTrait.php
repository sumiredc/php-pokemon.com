...
trait ServiceBattleAttackTrait
{
	...
	*/
	...
	/**
	* 攻撃
	* @param App\Classes\Pokemon $atk
	* @param App\Classes\Pokemon $def
	* @param string $move
	* @return string::Move
	*/
	protected function attack(Pokemon $atk, Pokemon $def, string $move) :string
	{
		// 行動判定前に前回選択された技を初期化
	...
	* @return boolean
	*/
	private function attackActivateMove(Pokemon $atk, Pokemon $def, string $move): bool
	{
		if(
	...
	*/
	private function attackConfirmMove(object $atk, object $def, string $move): void
	{
		...
		// 発動メッセージ
		response()->setMessage(
			$move::getTriggerMessage($atk->getPrefixName()),
			$this->atk_msg_id
		);
		// 「へんしん」の特別処理
	...
	*/
	private function attackSuccess($atk, $def, $move)
	{
		...
		battle_state()->setTurnDamage(
			$def->getPosition(), $move::SPECIES, $damage
		);
		// 変化技ではなく、ダメージ量が0の場合
		if(
			$move::SPECIES !== 'status' &&
			...
			response()->setMessage('しかし、うまく決まらなかった');
			return;
		}
		// がまんダメージの蓄積処理
		if($def->isSc('ScBide')){
			$def->setBideDamage($damage);
		}
		// ダメージ計算
		$def->calRemainingHp('sub', $damage);
		// HPバーのアニメーション用レスポンス
		if($damage){
			response()->setResponse([
				'param' => $damage,
		...
		if($def->getRemainingHp()){
			// 追加効果
			$effects = $this->effectMove($atk, $def, $move, $damage);
			// バトル終了
	...
	* @return array|null
	*/
	private function effectMove(Pokemon $atk, Pokemon $def, string $move, int $damage)
	{
		$effects = $move::effects($atk, $def, $damage);
		// メッセージが取得できたらセット
		if(
			...
			$effect_id = response()->issueMsgId();
			response()->setMessage($effects['message'], $effect_id);
			$atk->calRemainingHp('add', $effects['heal']);
			// HPバーのアニメーション用レスポンス
			if($effects['heal']){
				response()->setResponse([
					'param' => $effects['heal'] * -1,
					'action' => 'hpbar',
