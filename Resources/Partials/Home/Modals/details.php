                ...
                <div class="tab-content" id="pokemon-details-tabContent">
                    ...
                    <div class="tab-pane fade show" id="move" role="tabpanel" aria-labelledby="move-tab">
                        ...
                        <table class="table table-bordered table-sm table-hover">
                            ...
                            <tbody>
                                <?php foreach($pokemon->getMove() as $key => $move): ?>
                                    <tr class="move-detail-link" data-target="#move_<?=get_class($move)?>-content">
                        ...
                        <div class="overflow-auto p-3 border" style="height:160px;">
                            <?php foreach($pokemon->getMove() as $key => $move): ?>
                                <div class="move-detail-content <?php if(array_key_first($pokemon->getMove()) == $key) echo 'active'; ?>" id="move_<?=get_class($move)?>-content">
                                    <h6><?=$move->getName()?></h6>
                                    <hr>
                                    <p><?=$move->getDescription()?></p>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
