<section id="battle-field" class="p-1 p-sm-3 border-bottom">
    <div class="row mb-3 mb-sm-5 py-3 py-sm-4">
        <?php # 敵ポケモン詳細 ?>
        <div class="col-6 <?php if(!$before_enemy->isFight()) echo 'opacity-0'; ?>" id="enemy-pokemon-parameter">
            <p>
        ...
        <div class="col-6 text-center">
            ...
            class="action-img-btn <?php if(!$before_enemy->isFight()) echo 'opacity-0'; ?>"
    ...
    <div class="row py-3 py-sm-4">
        ...
        <div class="col-6 text-center">
            ...
            class="action-img-btn <?php if(response()->getResponse('battle-start') || !$before_friend->isFight()) echo 'opacity-0'; ?>"
            src="/Assets/img/pokemon/dots/back/<?=get_class($before_friend)?>.gif"
            alt="<?=$before_friend->getName()?>"
            data-pokemon="<?=get_class($before_friend)?>"
            data-toggle="modal"
            data-target="#friend-battle-state-modal" />
        </div>
        <div class="col-6 <?php if(response()->getResponse('battle-start') || !$before_friend->isFight()) echo 'opacity-0'; ?>" id="friend-pokemon-parameter">
            <p>
            ...
            <div class="form-group">
                <div class="progress rounded-pill bg-gray" style="height:12px;">
                    ...
                    aria-valuemin="0"
                    aria-valuemax="<?=$before_friend->getStats('HP')?>"></div>
                </div>
                <p class="text-right px-3">
                    <span id="remaining-hp-count-friend"><?=$before_friend->getRemainingHp()?></span>
                    / <span id="max-hp-count-friend"><?=$before_friend->getStats('HP')?></span>
                </p>
                <?php # 経験値バー ?>
